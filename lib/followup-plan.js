...
const FollowupPlan = (() => {
  ...
  async function rescheduleAfterMerge(entityType, entityId, mergedData) {
    ...
    if (entityType === 'patients') {
      patient = mergedData;
      visits = await DB.loadVisitsByPatient(entityId);
    } else if (entityType === 'visits') {
      patient = await DB.loadPatient(mergedData.patientId);
      if (!patient) return null;
      visits = await DB.loadVisitsByPatient(mergedData.patientId);
    } else {
      return null;
    }
    ...
    for (const plan of existingPlans) {
      if (plan.status === 'pending' || plan.status === 'overdue') {
        ...
        plan.cancelReason = 'conflict_merge_reschedule';
        plan.cancelledAt = Utils.now();
        await DB.savePlan(plan);
      }
    }
    ...
    const newPlan = await generatePlan(patient, visits, rules);
    newPlan.status = _checkOverdue(newPlan);
    newPlan._rescheduledFrom = 'conflict_merge';
    const saved = await DB.savePlan(newPlan);

    // 重排提醒
    let reminderResult = null;
    if (typeof Reminders !== 'undefined' && Reminders.rescheduleAfterMerge) {
      reminderResult = await Reminders.rescheduleAfterMerge(entityType, entityId, mergedData);
    }

    return {
      plan: saved,
      reminder: reminderResult,
      cancelledPlans: existingPlans.filter(p => p.status === 'cancelled').length
    };
  }

  return {
    generatePlan, generateAllPlans, processPostVisit,
    getPlanSummary, exportAuditData, rescheduleAfterMerge,
