...
const SyncManager = (() => {
  ...
  async function _queueAttachmentsForUpload(visitId, attachments) {
    for (const att of attachments) {
      if (att.compressionState === 'uploaded' || att.uploadStatus === 'uploaded') {
        continue; // 跳过已上传的附件
      }
      // addToAttachmentQueue 内部已做去重（按 attachmentId），此处不再额外检查
  ...
  function threeLevelMerge(base, local, remote, entityType) {
    ...
    // === 补访链路保护 ===
    // 如果本地有补访标记（missed_revisit/abnormal_revisit），
    // 合并结果应保留本地的补访链路数据，防止远程覆盖
    ...
    for (const field of _revisitFields) {
      ...
      if (localVal !== undefined && localVal !== null) {
        ...
        if (localHasRevisit && !remoteHasRevisit) {
          ...
          levelDetails.patient.push({
            field, choice: 'local-revisit-preserved', level: 'patient'
          });
  ...
  async function _handleConflict(item, error) {
    ...
    if (hasAutoResolvableConflicts) {
      ...
      // 重新加入同步队列以推送合并结果（保留重试状态，避免掩盖历史失败）
      await DB.addToSyncQueue(item.entityType, item.entityId, 'update', mergedRecord, {
        preserveRetryState: true
      });

      // 冲突合并后重新计算随访计划和提醒
      await _recalculateAfterMerge(item.entityType, item.entityId, mergedRecord);
    } else {
  ...
  // 冲突合并后重新计算随访计划和提醒
  async function _recalculateAfterMerge(entityType, entityId, mergedRecord) {
    try {
      let patientId = null;

      if (entityType === 'visits') {
        patientId = mergedRecord.patientId;
        // 访问记录合并后：重新生成该患者的随访计划和提醒
        if (patientId && typeof FollowupPlan !== 'undefined') {
          const patient = await DB.loadPatient(patientId);
          if (patient) {
            const visits = await DB.loadVisitsByPatient(patientId);
            await FollowupPlan.recalculatePlansForPatient(patient, visits);
          }
        }
        if (patientId && typeof Reminders !== 'undefined' && Reminders.recalculateReminders) {
          await Reminders.recalculateReminders();
        }
      } else if (entityType === 'patients') {
        patientId = entityId;
        // 患者记录合并后：风险等级可能变化，重新计算计划和提醒
        if (typeof FollowupPlan !== 'undefined' && FollowupPlan.recalculatePlansForPatient) {
          const patient = await DB.loadPatient(patientId);
          if (patient) {
            const visits = await DB.loadVisitsByPatient(patientId);
            await FollowupPlan.recalculatePlansForPatient(patient, visits);
        ...
        if (typeof Reminders !== 'undefined' && Reminders.recalculateReminders) {
          await Reminders.recalculateReminders();
        }
      }

      await DB._logOperation(entityType, entityId, 'post_merge_recalc', {
        patientId,
        entityType,
        mergedFields: Object.keys(mergedRecord).filter(k => !k.startsWith('_'))
      });
    } catch (e) {
      console.warn('Post-merge recalculation failed:', e);
    }
  }
  ...
  async function resolveConflict(conflictId, resolvedData, choice) {
    ...
    // 重新加入同步队列推送合并结果（保留重试状态）
    await DB.addToSyncQueue(conflict.entityType, conflict.entityId, 'update', record, {
      preserveRetryState: true
    });

    // 手动解决冲突后也触发计划和提醒重算
    await _recalculateAfterMerge(conflict.entityType, conflict.entityId, record);

    await DB._logOperation(conflict.entityType, conflict.entityId, 'conflict_resolved', {
      conflictId,
      choice,
