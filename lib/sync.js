...
const SyncManager = (() => {
  ...
  async function _handleConflict(item, error) {
    ...
    // 执行三方合并
    let mergeResult;
    if (item.entityType === 'visits' && localData?._templateVersions) {
      mergeResult = threeWayMergeWithTemplate(
        baseSnapshot, localData, remoteData,
        { baseVersion: localData?._templateVersions?.[0]?.version }
      );
    } else {
  ...
  return {
    ...
    threeWayMerge, threeWayMergeWithTemplate,
    isSyncing: () => _syncing,
