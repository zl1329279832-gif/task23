...
const DB = (() => {
  ...
  const STORES = {
    ...
    attachment_queue: { keyPath: 'id', indexes: ['visitId', 'status', 'attachmentId'] },
    followup_plans: { keyPath: 'id', indexes: ['patientId', 'status', 'planDate', 'priority'] },
    risk_config: { keyPath: 'id', indexes: ['priority'] },
    _operation_log: { keyPath: 'id', indexes: ['entityType', 'entityId', 'timestamp'] }
  };
  ...
  async function savePatient(patient) {
    ...
    await _logOperation('patients', patient.id, isNew ? 'create' : 'update', {
      ...
      hasBaseSnapshot: !!patient._baseSnapshot
    });
  ...
  async function saveVisit(visit) {
    ...
    await _logOperation('visits', visit.id, isNew ? 'create' : 'update', {
      ...
      hasQuestionnaires: !!(visit.questionnaires && visit.questionnaires.length > 0),
      hasAttachments: !!(visit.attachments && visit.attachments.length > 0),
  ...
  async function addToSyncQueue(entityType, entityId, action, payload, options = {}) {
    ...
    if (existingForEntity.length > 0) {
      ...
      // 仅当非「保留重试状态」模式时才重置重试计数
      // 冲突解决后重新入队不应重置 retryCount（防止掩盖历史失败）
      if (!preserveRetryState) {
        existing.retryCount = 0;
        existing.lastError = null;
        existing.syncStatus = null; // 清除 failed 标记
      }
      ...
      await _logOperation(entityType, entityId, 'queue_update', {
    ...
    await _logOperation(entityType, entityId, 'queue_add', {
      action, queueItemId: item.id
    });
  ...
  async function addToAttachmentQueue(attachment) {
    ...
    if (attachment.id) {
      const existingByAttId = await getByIndex(db, 'attachment_queue', 'attachmentId', attachment.id);
      const existingActive = existingByAttId.filter(
        item => item.status !== 'uploaded' && item.status !== 'failed'
      );
      if (existingActive.length > 0) {
        // 已存在且仍在处理中 → 更新数据但保留重试状态
      ...
      // 检查是否有已失败的记录 → 允许重新入队但重置状态
      const existingFailed = existingByAttId.filter(item => item.status === 'failed');
  ...
  async function getPlansByPriority(priority) {
    const db = await open();
    return getByIndex(db, 'followup_plans', 'priority', priority);
  }
  ...
  async function getRiskConfig() {
    const db = await open();
    const rules = await getAll(db, 'risk_config');
    return rules.sort((a, b) => (a.priority || 99) - (b.priority || 99));
  }

  async function getRiskConfigById(id) {
    const db = await open();
    return get(db, 'risk_config', id);
  }

  async function deleteRiskConfig(id) {
    const db = await open();
    await deleteRecord(db, 'risk_config', id);
  }
  ...
  async function getOperationLog(entityType, entityId) {
    const db = await open();
    if (entityId) {
      const items = await getByIndex(db, '_operation_log', 'entityId', entityId);
      return items.filter(i => !entityType || i.entityType === entityType)
        .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
    }
    return (await getAll(db, '_operation_log'))
      .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
  }

  async function clearOperationLog(beforeTimestamp) {
    const db = await open();
    if (!beforeTimestamp) {
      await clear(db, '_operation_log');
      return;
    }
    const items = await getAll(db, '_operation_log');
    for (const item of items) {
      if (new Date(item.timestamp) < new Date(beforeTimestamp)) {
        await deleteRecord(db, '_operation_log', item.id);
      }
    }
  }
  ...
  async function exportAuditLog() {
    ...
    return {
      ...
      version: '1.0',
      operationLog: ops.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)),
      ...
      pendingSyncQueue: queue.map(q => ({
        ...
        syncStatus: q.syncStatus || null,
        createdAt: q.createdAt,
        updatedAt: q.updatedAt
      })),
      pendingAttachments: attQueue
  ...
  return {
    ...
    savePlan, getPlan, getAllPlans, getPlansByPatient, getPlansByStatus, getPlansByPriority, deletePlan,
    saveRiskConfig, getRiskConfig, getRiskConfigById, deleteRiskConfig, clearRiskConfig,
    getOperationLog, clearOperationLog, exportAuditLog, _logOperation,
    _cleanSnapshot
  };
})();
